...
//ensure physics isn't run too often
...
__task void bomb (void) {
    ...
    while(1) {
        if(HUD.bombs < 1) {
			...
			// printf("Launch Bomb\n");
            //TODO do bomb stuff
            //TODO reduce life of all remaining asteroids by 200. Give points
            //printf("bomb firing\n");
        }
        os_dly_wait(bomb_button_delay);
    }
		...
		while((asteroids->bitVector != 0) && (HUD.lives >= 0) && (HUD.reset == 0)) {
			...
			if (HUD.lives < 0) {
				...
				os_tsk_create (GameOver, 254);
					...
					// //becareful of deadlock. Take the physics mutex since that one steps on a lot of data
					// //os_mut_wait(mut_physics, 0xffff);
					// os_mut_wait(mut_ship, 0xffff);
					// os_mut_wait(mut_bullets, 0xffff);
					// os_mut_wait(mut_asteroids, 0xffff);
					// //scrap all object data and start over for a new game
					// initializeEntities();
					// os_mut_release(mut_asteroids);
